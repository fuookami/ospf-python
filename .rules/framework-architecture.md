...
本规范适用于 `ospf_python.framework.*` 下以优化建模为核心的 framework 模块，包括但不限于 CSP1D、BPP3D、Gantt Scheduling 等领域框架。
...
1. 对外领域模型、配置和结果优先使用 `TypeVar('V', bound=RealNumber)` 等泛型数值抽象。
2. 宽度、长度、重量、面积、需求、产出、余料、容量等领域量应优先使用 `Quantity`、`PhysicalUnit` 或明确的物理量类型表达。
3. `float`、`int`、无单位裸值只能作为 solver adapter、registration、extraction 或低层数学接口的实现细节。
...
2. 可变规则优先通过 strategy、context、pipeline、adapter、函数参数、Protocol 或领域 policy 注入。
...
3. 当 application 中出现大量 `model.add(...)`、`model.add_constraint(...)`、目标项拼接或 token 解析逻辑时，应优先下沉到 domain context、aggregation、model component 或 pipeline。

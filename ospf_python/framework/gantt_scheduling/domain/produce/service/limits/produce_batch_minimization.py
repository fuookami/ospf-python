"""生产批次数量最小化 / Produce batch minimization.

生成最小化生产批次数量的目标函数数据，减少批次总数以
降低切换和管理成本。
Generates objective function data for minimizing the number
of production batches, reducing total batch count to lower
switching and management costs.
"""
...
class BatchObjectiveTerm:
    ...
    表示目标函数中一个批次数量的加权项。
    Represents a weighted term of batch count in the
    objective function.
...
class ProduceBatchMinimization:
    ...
    构建最小化生产批次使用数量的目标函数项，鼓励调度方案
    使用更少的批次。适用于希望合并小批次、减少批次切换次数
    的场景。
    Builds objective function terms for minimizing the number
    of production batches used, encouraging scheduling plans
    with fewer batches. Applicable when the goal is to merge
    small batches and reduce batch switching frequency.
    ...
    objective_name: str = "produce_batch_min"

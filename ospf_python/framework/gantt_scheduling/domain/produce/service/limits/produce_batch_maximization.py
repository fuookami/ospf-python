"""更优生产批次最大化 / Produce batch maximization.

生成最大化生产批次质量的目标函数数据，优先选择质量更高的
批次配置。
Generates objective function data for maximizing production
batch quality, preferring higher-quality batch configurations.
"""
...
class QualityObjectiveTerm:
    ...
    表示目标函数中一个批次质量的加权项。
    Represents a weighted term of batch quality in the
    objective function.
...
class ProduceBatchMaximization:
    ...
    构建最大化生产批次质量的目标函数项，鼓励调度方案选择
    质量得分更高的批次配置。适用于存在多种可选批次方案且
    希望优选最佳方案的场景。
    Builds objective function terms for maximizing production
    batch quality, encouraging scheduling plans that select
    higher-quality batch configurations. Applicable when
    multiple batch configurations are available and the best
    one is preferred.
    ...
    objective_name: str = "produce_batch_max"
    ...
    def objective_name_for(
        ...
        Generate batch-specific objective function name.

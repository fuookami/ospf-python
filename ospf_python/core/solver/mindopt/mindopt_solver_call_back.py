"""MindOPT 求解器回调处理器 / MindOPT solver callback
handler.

管理 MindOPT 求解过程中的回调事件。
Manages callback events during MindOPT solving.
"""
...
class MindOPTSolverCallBack:
    """MindOPT 求解器回调处理器 / MindOPT solver callback
    handler.

    冻结数据类，封装 MindOPT 回调函数的注册和管理逻辑。
    Frozen dataclass encapsulating callback registration
    and management logic for MindOPT.
